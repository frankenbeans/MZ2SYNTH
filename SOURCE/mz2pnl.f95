...
! Copyright (C) 2004 by E. Lamprecht - All rights reserved.  
! ------------------------------------------------------------------------------
...
MODULE MZ2Pnl
  ...
  TYPE :: MZ2Panel
     ...
     REAL(KIND=RKIND)         :: DKTIMC     =1.0_RKIND/(48000.0_RKIND/100)
  ...
  SUBROUTINE MZ2Pnl_Load(P,IFN,CHS,SCANRT,SMPLRT,TCFRAC)
    ...
    CASE('L')
       IF (.NOT.ASSOCIATED(DLUM)) THEN
          ...
          IF (MS.NE.0) GOTO 910
          !$OMP PARALLEL
          WHERE(P%PI%DRED.GT.0.OR.P%PI%DGRN.GT.0.OR.P%PI%DBLU.GT.0)
             DLUM=Luminance(P%PI%DRED,P%PI%DGRN,P%PI%DBLU)
          ELSEWHERE
             DLUM=0
          END WHERE
          !$OMP END PARALLEL
    ...
    P%DKTIMC=1.0_RKIND/(TCFRAC*P%SMPLRT)
    ! --- END CODE ---
    RETURN
...
930 WRITE(*,700) 'Invalid TCPERC'                                         ; STOP
  ...
  SUBROUTINE MZ2Pnl_Tick(P,DONE)
    ...
    ! --- VARIABLES ---
    LOGICAL          :: DOSINE,DOSQWV,DOSWTH,DOTRNG
    INTEGER :: CC,NS,J
    ! --- EXE CODE ---
    DONE=.FALSE.
    NS=P%CS+1
    ...
    END IF
    P%CS=NS
    DOSINE=ASSOCIATED(P%DTSINE)
    DOSQWV=ASSOCIATED(P%DTSQWV)
    DOSWTH=ASSOCIATED(P%DTSWTH)
    DOTRNG=ASSOCIATED(P%DTTRNG)
    IF (DOSINE) THEN
       !$OMP PARALLEL DO
       DO J=1,P%PI%NROWS
          CALL RM_TICK(P%ASINE(J),P%WSINE(J),P%DKTIMC,P%DTSINE(CC,J),P%VZERO)
       END DO       
       !OMP END PARALLEL DO
    END IF
    IF (DOSQWV) THEN
       !$OMP PARALLEL DO
       DO J=1,P%PI%NROWS
          CALL RM_TICK(P%ASQWV(J),P%WSQWV(J),P%DKTIMC,P%DTSQWV(CC,J),P%VZERO)
       END DO       
       !OMP END PARALLEL DO
    END IF
    IF (DOSWTH) THEN
       !$OMP PARALLEL DO
       DO J=1,P%PI%NROWS
          CALL RM_TICK(P%ASWTH(J),P%WSWTH(J),P%DKTIMC,P%DTSWTH(CC,J),P%VZERO)
       END DO       
       !OMP END PARALLEL DO
    END IF
    IF (DOTRNG) THEN
       !$OMP PARALLEL DO
       DO J=1,P%PI%NROWS
          CALL RM_TICK(P%ATRNG(J),P%WTRNG(J),P%DKTIMC,P%DTTRNG(CC,J),P%VZERO)
       END DO       
       !OMP END PARALLEL DO
    END IF    
    ! --- END CODE ---
  ...
  CONTAINS
    ...
    PURE SUBROUTINE RM_TICK(RMA,RMW,CDC,RMI,VZR)
      IMPLICIT NONE
